...
* :mod:`mdg_drawio.reverse.scoring` -- weighted token similarity. Shape-defining
...
* :mod:`mdg_drawio.reverse.derive` -- document-level ranking. Unambiguous cells
  ...
  shape -> uml).
* :mod:`mdg_drawio.reverse.naming` -- assigns each resolved cell a semantic
  ``.mdg`` node id (``person1``, ``system1``, ...) from its shape's registry
  slug, so a derived diagram's ids read like a hand-authored one.
* :mod:`mdg_drawio.reverse.containment` -- resolves where each cell nests (its
...
* :mod:`mdg_drawio.reverse.merge` -- splices new cells into an EXISTING ``.mdg``
  ...
  ``mdg_drawio/reverse/merge_cli.py`` for the dry-run-by-default,
  validate-before-write CLI, wired up as the ``mdg merge`` subcommand
  (:mod:`mdg_drawio.cli`).
